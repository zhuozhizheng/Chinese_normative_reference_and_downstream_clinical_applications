...
var_pre<-c('MMSE','MOCA','BVLT','CVLT','SDMT','PASAT',
           'Pre_UPDRS_III_med_off','EDSS')

# var_pre<-c('DBS_Improvement_difference_off','DBS_improvement_rate_off',
# 'Bergt_difference')
...
p<-ggplot(tem_list_fig, aes(label = MRI_feature,
  ...
  theme_minimal()+
  scale_color_gradient2(limits=c(min(tem_list_fig$rvalue),max(tem_list_fig$rvalue)),low = "#00CCFF",mid="white",high = "#990000")
print(p)
dev.off()
...
# tem_data0<-tem_data[,c(var_pre[i_var],feature_col,'Diagnosis')];
# tem_data0<-na.omit(tem_data0);
# tem_data0_zz<-tem_data0;
# 
# tem_data0[,'Diagnosis']<-tem_data0_zz$Diagnosis
# 
# png(filename = paste0(i_var,'_all_disease_clinical_association.png'),
#     width = 2000,
#     height = 1480,
#     units = "px",
#     bg = "white",
...
#     res = 300)
# 
# p<-ggplot(tem_data0,aes(x=label,y=predict,color=Diagnosis))+
#   #geom_point(size=2)+
#   geom_point(aes( color = Diagnosis), size=1)+
...
write.csv(list_all_disease,'clinical_association_all_diseases.csv')
saveRDS(results,'clinical_association.rds')

#plot list_all_disease
png(filename = paste0('heatmap_clinical_association_step.png'),
    width = 2000,
    height = 1480,
    units = "px",
    bg = "white",
...
png(filename = paste0('heatmap_clinical_association_adjust_R2_step.png'),
    width = 3200,
    height = 1880,
    units = "px",
    bg = "white",
...
#plot list_all_disease
png(filename = paste0('heatmap_clinical_association_correlation_R_map_step.png'),
    width = 2500,
    height = 1500,
    units = "px",
    bg = "white",
...
heatmap=ggplot(data=list_all_disease,aes(x=disease,y=clinical_feature,fill=round(rvalue,2)))+
  ...
  scale_y_discrete(labels = c("relapses" = "Relapse Number","MOCA" = "MoCA", "BVLT"="BVMT","Post_DBS_UPRDS_off" = "UPRDS Receiving DBS",
                            "Pre_UPDRS_III_med_on" = "UPRDS Receiving Medicine","Pre_UPDRS_III_med_off" = "UPRDS"))
print(heatmap)
dev.off()

png(filename = paste0('heatmap_clinical_association_correlation_mape_step.png'),
    width = 2500,
    height = 1500,
    units = "px",
    bg = "white",
