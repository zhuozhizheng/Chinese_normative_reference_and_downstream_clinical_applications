...
rad_dataz<-data.frame(matrix(NA,length(disease),length(disease)+2)) 
colnames(rad_dataz)<-c('Model','HC','MCI','AD',"PD","CSVD","MS","NMOSD")
rownames(rad_dataz)<-c(disease);
rad_dataz[,1]<-c('MCI','AD',"PD","CSVD","MS","NMOSD")
...
for(i in c(1:k))
{
  ...
  if(i<k)
  {
    indexk<-sample(x=pool,size = seg,replace = F)
    pool<-setdiff(pool,indexk)
    patient_ID<-rownames(tem_data0)[indexk]
    
    testing<-tem_data0[indexk,]
    training<-tem_data0[-indexk,]
  }
  ...
  tune_result<-tune(svm,as.factor(label)~.,data=training,kernel='radial',type='C',probability = TRUE,
                    ranges=list(cost=c(0.1,1,10),gama=c(0.1,1,10)))
  
  best_model2<-tune_result$best.model
  
  model[[disease[num]]]<-best_model2
...
predict2 <- predict(best_model,testing,probability = TRUE)
if(k==1){predict2 <- predict(best_model,training,probability = TRUE)}
predict2<-data.frame(attr(predict2, "probabilities")[,1])
rownames(predict2)<-patient_ID
predict_value<-rbind(predict_value,predict2)
...
tem_dataz<-cbind(tem_dataz,tem_predict);
colnames(tem_dataz)[dim(data)[2]+i]<-paste0('pre_',i);

}

tem_data1[,dim(data)[2]+num]<-data.frame(rowSums(tem_dataz[,c((dim(data)[2]+1):(dim(data)[2]+k))])/k)
colnames(tem_data1)[dim(data)[2]+num]<-paste0(disease[num]);


predict.<-predict_value[rownames(tem_data0),]
...
rad_dataz[num,2:(length(disease)+2)]<-rad_data0['Value',]
...
rad_dataz$Model<-factor(rad_dataz$Model,levels=c('MCI','AD',"PD","CSVD","MS","NMOSD"))

png(filename = paste0(str,'_radar_plot_all_diseases',".png"),
    width = 3480,
    height = 3480,
    units = "px",
    bg = "white",
...
p<-ggradar::ggradar(rad_dataz,
                    ...
                    axis.label.size = 10,
                    group.point.size = 10,
                    group.colours = c("#F8766D", "#B79F00","#00BA38",
...
p<-ggroc(list_roc,linetype='solid',linewidth=1,alpha=1)+
...
#}
print(list_stats)
write.csv(list_stats,'DPS_results.rds')
write.csv(list_roc_results,'DPS_results_roc.rds')
