...
feature_seg[feature_seg=='Left-Thalamus']<-c("Left-Thalamus-Proper")
feature_seg[feature_seg=='Right-Thalamus']<-c("Right-Thalamus-Proper")
...
library(ggsci)
setwd(savepath)
png(filename = paste0(str_lab,'_',dis,'_','aseg_CIdif.png'),
    width = 1200,
    height = 850,
    units = "px",
    bg = "white",
...
p<-ggplot(seg_data)+
  geom_brain(atlas=aseg,
             ...
             mapping = aes(fill = CI))+
  theme_void()+
  #scale_color_continuous(name='CI')+
...
library(ggsci)
setwd(savepath)
png(filename = paste0(str_lab,'_',dis,'_','aseg.png'),
    width = 1200,
    height = 850,
    units = "px",
    bg = "white",
...
p<-ggplot(seg_data)+
  geom_brain(atlas=aseg,
             ...
             mapping = aes(fill = d))+
  theme_void()+
  #scale_color_continuous(name="Cohen's d")+
